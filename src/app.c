...
{	
	...
	while (1)
	{
		if((robotMode==Forward)||(robotMode==ToRight)||(robotMode==ToLeft)||(robotMode==ToStop))
		{
			/* 机器人步行循环分为4步，每一步开始都要先进行初始化，每步仅执行1次初始化 */
			if(flagStart==1) //此标志位用于保证仅执行一次初始化      
			{								
				/****************************************** 左腿初始化 **************************************************/
				/*##-1- 计算开机时膝关节位置与其目标位置的差值 */
				temp = myRobot.actionctrl.leftLeg.positionDest_Float - myRobot.actionctrl.leftLeg.positionNow_Float;
				
				/*##-2- 计算每步应走的值 */
				myRobot.actionctrl.leftLeg.step = (float)temp/(float)WR;//WR: Walk Resolution(it is a global variable in "app.c")
				
				/*##-3- 初始化下一个目标点的值为当前膝关节位置的值 */
				myRobot.actionctrl.leftLeg.positionNext_Float  = myRobot.actionctrl.leftLeg.positionNow_Float;
				
				/*##-4- 保存当前膝关节位置的值 */
				myRobot.actionctrl.leftLeg.positionStart_Float = myRobot.actionctrl.leftLeg.positionNow_Float;
				
				/******************************************* 右腿初始化 ************************************************************/
				/* 步骤同左腿 */
				temp = myRobot.actionctrl.rightLeg.positionDest_Float - myRobot.actionctrl.rightLeg.positionNow_Float;
				myRobot.actionctrl.rightLeg.step=(float)temp/(float)WR;//WR: Walk Resolution(it is a global variable in "app.c")
				myRobot.actionctrl.rightLeg.positionNext_Float  = myRobot.actionctrl.rightLeg.positionNow_Float;
				myRobot.actionctrl.rightLeg.positionStart_Float = myRobot.actionctrl.rightLeg.positionNow_Float;
...
// 				WRC++;
				if(WRC>=WR)
				{
					...
					{			
						if(robotMode==ToStop)
						{
							...
							//机器人stop状态，停止挥臂动作并复位手臂
							actionFlag = 0;	//停止挥臂
							calibrationLFlag = 0;	//复位左臂
							calibrationRFlag = 0; //复位右臂
				...
				{					
					/* 左脚 (TIM5_CH2 PA1 No Remap)
					 * 向前走：pwmSet < 50%
					 * 向后走：pwmSet > 50%
					 */
					if(myRobot.actionctrl.leftLeg.step>0)//如果左腿向前迈，则脚上跟着往前走
					{
//						pwmSet = (100-pwset*2)*(52*2*pi/60)*0.2*cos(arctan((fabs(myRobot.actionctrl.leftLeg.positionNow_Float)/1000)/0.045))/(pi*(float)RPM*(float)D)     //add by cao
						LeftFootMove(FootSpeed);//其速度视膝关节运动速度而定
//						LeftFootMove(14);//其速度视膝关节运动速度而定
					}
					else
					{
						LeftFootMove(50);
					...
					/* 右脚 (TIM3_CH2 PB5 Remap)
					 * 向前走：pwmSet < 50%
					 * 向后走：pwmSet > 50%
					 */
					if(myRobot.actionctrl.rightLeg.step>0)//如果右腿向前迈，则右脚上跟着往前走
					{			
//						pwmSet = 50 - (short)((400.0*sin(THEANGLE/180.0*Pi)/((float)RPM/60.0 * Pi *(float)D*((float)RESOLUTION*(float)X/1000.0)))*50);
 						RightFootMove(FootSpeed);//其速度视膝关节运动速度而定
					}
					else
					{
						RightFootMove(50);
					}
					/* 左膝 (TIM5_CH1 PA0 No Remap)
					 * 向前屈：pwmSet < 50%
					 * 向后屈：pwmSet > 50%
					 */
					myRobot.actionctrl.leftLeg.positionNext_Float = myRobot.actionctrl.leftLeg.positionStart_Float + myRobot.actionctrl.leftLeg.step*WRC;
					DIF_Value = myRobot.actionctrl.leftLeg.positionNext_Float - myRobot.actionctrl.leftLeg.positionNow_Float;
					pwmSetL = 50-DIF_Value*POWER;//
					/* 此方案需要 “ 1路PWM产生2对互补PWM ” 方案的支持  */
					//pwmSet = 50-(myRobot.actionctrl.leftLeg.positionDest_Float - myRobot.actionctrl.leftLeg.positionNow_Float)*5;
					if(myRobot.actionctrl.leftLeg.step < 0.0)
					{
						if(DIF_Value < -0.5)
						{
							pwmSetL=pwmSetL+8;
					...
					else if(myRobot.actionctrl.leftLeg.step > 0.0)
					{
						if(DIF_Value > 0.5)
						{
							pwmSetL=pwmSetL-8;
					...
					/* 右膝 (TIM3_CH1 PB4 Reamp)
					 * 向前屈：pwmSet < 50%
					 * 向后屈：pwmSet > 50%
					 */
					myRobot.actionctrl.rightLeg.positionNext_Float = myRobot.actionctrl.rightLeg.positionStart_Float + myRobot.actionctrl.rightLeg.step*WRC;
					DIF_Value = myRobot.actionctrl.rightLeg.positionNext_Float - myRobot.actionctrl.rightLeg.positionNow_Float;
					pwmSetR = 50-DIF_Value*POWER;//一共100个数据
					/* 此方案需要 “ 1路PWM产生2对互补PWM ” 方案的支持  */
// 					pwmSet = 50-(myRobot.actionctrl.rightLeg.positionDest_Float - myRobot.actionctrl.rightLeg.positionNow_Float)*5;
					if(myRobot.actionctrl.rightLeg.step < 0.0)
					{
						if(DIF_Value < -0.5)
						{
							pwmSetR=pwmSetR+8;
					...
					else if(myRobot.actionctrl.rightLeg.step > 0.0)
					{
						if(DIF_Value > 0.5)
						{
							pwmSetR=pwmSetR-8;
...
// 					OS_ENTER_CRITICAL();				//进入临界区	
					LeftKneeMove(pwmSetL);
					RightKneeMove(pwmSetR);						
// 					OS_EXIT_CRITICAL();					//退出临界区		
		...
		/***********************************Robot*Backward***************************************/		
		else if( (robotMode==Backward) || (robotMode==ToSlideBackward))
		{
			if(flagStart_Backward==1)      //如果刚开始第一次
			{	
				//左腿初始位置
				temp = myRobot.actionctrl.leftLeg.positionDest_Float - myRobot.actionctrl.leftLeg.positionNow_Float;
				myRobot.actionctrl.leftLeg.step = (float)temp/(float)WR;//每步要走的角度
				myRobot.actionctrl.leftLeg.positionNext_Float  = myRobot.actionctrl.leftLeg.positionNow_Float;
				myRobot.actionctrl.leftLeg.positionStart_Float = myRobot.actionctrl.leftLeg.positionNow_Float;
				
				//右腿初始位置
				temp = myRobot.actionctrl.rightLeg.positionDest_Float - myRobot.actionctrl.rightLeg.positionNow_Float;
				myRobot.actionctrl.rightLeg.step=(float)temp/(float)WR;//每步要走的角度
				myRobot.actionctrl.rightLeg.positionNext_Float  = myRobot.actionctrl.rightLeg.positionNow_Float;
				myRobot.actionctrl.rightLeg.positionStart_Float = myRobot.actionctrl.rightLeg.positionNow_Float;
				
				WRC=0;
				flagStart_Backward=0;
			}
			if(actionStart_Backward==1)
			{
				//如果开始运动了
// 				WRC++;
				if(WRC>=WR)
				...
				else
				{
					...
					myRobot.actionctrl.leftLeg.positionNext_Float = myRobot.actionctrl.leftLeg.positionStart_Float + myRobot.actionctrl.leftLeg.step*WRC;
					DIF_Value=myRobot.actionctrl.leftLeg.positionNext_Float - myRobot.actionctrl.leftLeg.positionNow_Float;
					pwmSetL=50-DIF_Value*POWER;//一共100个数据
					if(myRobot.actionctrl.leftLeg.step < 0.0)
					{
						if( DIF_Value < -0.5)
						{
					...
					if(myRobot.actionctrl.leftLeg.step<0)//如果左腿向前迈，则脚上跟着往前走
					{
// 						pwmSet=50-(myRobot.actionctrl.leftLeg.positionNext_Float - myRobot.actionctrl.leftLeg.positionNow_Float)*POWER;
// 						if(pwmSet>99){
// 							pwmSet=99;
// 						}
						LeftFootMove(FootSpeedBack);
					}else{
						LeftFootMove(50);
					}
					...
					if(myRobot.actionctrl.rightLeg.step<0)//如果右腿向前迈，则右脚上跟着往前走
					{
// 						pwmSet=50-(myRobot.actionctrl.rightLeg.positionNext_Float - myRobot.actionctrl.rightLeg.positionNow_Float)*POWER;
// 						if(pwmSet>99){
// 							pwmSet=99;
// 						}
						RightFootMove(FootSpeedBack);
					}else{
						RightFootMove(50);
					}
					OS_EXIT_CRITICAL();					//退出临界区
...
/*******************************************************************************/
		...
		else if(robotMode==TurnLeft)
		{
			LeftFootMove(45);
			RightFootMove(5);
//			robotMode = NoTaskMode; //使机器人进入无任务模式，防止PWM重复设定(此情景下PWM只需设定一次)
		}else if(robotMode==TurnRight)
		{
			LeftFootMove(5);
			RightFootMove(45);
//			robotMode = NoTaskMode;
		}else if(robotMode==SlideForward)
		{
			LeftFootMove(5);
			RightFootMove(5);
//			robotMode = StopButNoResetRobot;
		}else if(robotMode==SlideBackward)
		{
			LeftFootMove(95);
			RightFootMove(95);
//			robotMode = NoTaskMode;
		}else if(robotMode==Stop)
		{
			LeftFootMove(50);
			LeftKneeMove(50);
			LeftArmMove(50);
